...
async function abrirJuego(juego){
    ...
    if (juego === 'menumisterioso') iniciarMenuMisterioso();
    if (juego === 'semaforo') iniciarSemaforo();
    if (juego === 'serenata') iniciarSerenata();
...
function renderMenuPrincipal(){
    ...
    if (gridCat && window.CATEGORIAS) {
        gridCat.innerHTML = window.CATEGORIAS.map(c => {
            ...
            return `<a class="tarjeta-categoria" href="categorias/${c.id}.html" style="--color-cat:${c.color};">
